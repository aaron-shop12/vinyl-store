...
class Utilities {
	...
	function clear_ignored_queries() {
		...
		// SearchWP v3 compatibility. Clears ignored queries for the current user.
		if ( class_exists( 'SearchWP_Stats' ) ) {
			update_user_meta( get_current_user_id(), SEARCHWP_PREFIX . 'ignored_queries', array() );
			wp_send_json_success();
		}

		// SearchWP v4 compatibility. Clears ignored queries for all users.
